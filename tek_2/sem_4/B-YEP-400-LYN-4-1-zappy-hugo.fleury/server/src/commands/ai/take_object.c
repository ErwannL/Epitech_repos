...
int retrieve_object_by_name(char *object)
{
    ...
    for (int i = 0; i < 7; i++)
        if (my_contains(object, resources[i]))
            key = i;
    return (key);
}

int retrieve_ressource(my_ftp_t *myftp, user_t *user, int pos[2], char *obj)
{
    char *resources[7] = {"food", "linemate", "derauemere", "sibur"\
    , "mendiane", "phiras", "thystame"};
    int key = 0;
...
int check_case_player(my_ftp_t *myftp, user_t *user, int pos[2], char *object)
{
    int r_value = 0;
    ...
    player = player->next) {
        if (player == user) {
            r_value = retrieve_ressource(myftp, player, pos, object);
        }
    }
    return (r_value);
}

int handle_take_object(my_ftp_t *myftp, user_t *user, char *input_str)
{
    ...
    for (int i = 0; i < myftp->config->width; i++) {
        for (int j = 0; j < myftp->config->height; j++) {
            r_value = check_case_player(myftp, user, (int [2]) {i, j}, value);
        }
    }
