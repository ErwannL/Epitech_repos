...
int flag_function(var_t *var, int ac, char **av)
{
    for (int i = 1; i != ac; i++) {
    ...
    if (test_error_flag(var) == 84) {
        dprintf(2, "invalid argument\n");
        return 84;
    } else
        flag_function_bonus(var);
    return 0;
}
...
int main(int ac, char **av)
{
    ...
    for (int i = 0; i != ac; i++)
        if (strcmp(av[i], "-h") == 0 || strcmp(av[i], "-d") == 0
        || strcmp(av[i], "-cf") == 0 || strcmp(av[i], "-ct") == 0)
            return print_h(var, av[i], av, ac);
    if (ac < 3 || ac > 9) {
        free(var);
        return 84;
    }
    r = flag_function(var, ac, av);
    free(var);
    return r;
}
